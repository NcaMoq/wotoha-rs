...
WOTOHA_LOUDNESS_NORMALIZATION_ENABLED=true
WOTOHA_LOUDNESS_TARGET_LUFS=-16.0
WOTOHA_LOUDNESS_MAX_BOOST_DB=6.0
WOTOHA_LOUDNESS_TRUE_PEAK_CEILING_DBTP=-2.0
...
曲ごとの音量差を補正するラウドネス正規化は既定で有効です。`WOTOHA_LOUDNESS_TARGET_LUFS` は `-24.0..=-8.0` LUFS、`WOTOHA_LOUDNESS_MAX_BOOST_DB` は `0.0..=12.0` dB、`WOTOHA_LOUDNESS_TRUE_PEAK_CEILING_DBTP` は `-6.0..=0.0` dBTP を受け付けます。`WOTOHA_LOUDNESS_NORMALIZATION_ENABLED=false` で無効化できます。
