...
struct StoredConnection {
    ...
    channel_id: u64,
    #[serde(default)]
    playback: Option<StoredPlayback>,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
struct StoredPlayback {
    current_source_url: String,
    queued_source_urls: Vec<String>,
    position_millis: u64,
    looping: bool,
...
impl ReconnectStore {
    ...
    pub(crate) fn save(
        &self,
        connections: &[(GuildKey, ChannelKey, Option<PlaybackRestartSnapshot>)],
    ) -> io::Result<()> {
        ...
        let state = ReconnectState {
            ...
            connections: connections
                .iter()
                .map(|(guild_id, channel_id, playback)| StoredConnection {
                    guild_id: guild_id.get(),
                    channel_id: channel_id.get(),
                    playback: playback.as_ref().map(|snapshot| StoredPlayback {
                        current_source_url: snapshot.current_source_url.clone(),
                        queued_source_urls: snapshot.queued_source_urls.clone(),
                        position_millis: snapshot.position.as_millis().min(u64::MAX as u128) as u64,
                        looping: snapshot.looping,
                        automix_enabled: snapshot.automix_enabled,
                    }),
                })
                .collect(),
        };
    ...
    pub(crate) fn take(
        &self,
    ) -> io::Result<Vec<(GuildKey, ChannelKey, Option<PlaybackRestartSnapshot>)>> {
        ...
        Ok(state
            ...
            .map(|entry| {
                let playback = entry.playback.map(|snapshot| PlaybackRestartSnapshot {
                    current_source_url: snapshot.current_source_url,
                    queued_source_urls: snapshot.queued_source_urls,
                    position: std::time::Duration::from_millis(snapshot.position_millis),
                    looping: snapshot.looping,
                    automix_enabled: snapshot.automix_enabled,
                });
                (
...
mod tests {
    ...
    fn save_then_take_is_one_shot() {
        ...
        let expected = vec![
            (GuildKey::new(10), ChannelKey::new(20), None),
            (
                GuildKey::new(30),
                ChannelKey::new(40),
                Some(PlaybackRestartSnapshot {
                    current_source_url: "https://example.com/current".to_owned(),
                    queued_source_urls: vec!["https://example.com/next".to_owned()],
    ...
    fn empty_save_removes_an_old_handoff() {
        ...
        let store = ReconnectStore::new(path.clone());
        store
            .save(&[(GuildKey::new(10), ChannelKey::new(20), None)])
            .unwrap();
